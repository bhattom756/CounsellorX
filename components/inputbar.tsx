...
const InputBar = () => {
  ...
  const handleSend = async () => {
    ...
    if (messages.length === 0) {
      ...
      setMessages((prev) => [...prev, { role: 'assistant', content: 'Please select from a case:' }])
    } else if (selectedCase && !showNatureOptions && messages.length <= 2) {
      setShowNatureOptions(true)
      setMessages((prev) => [...prev, { role: 'assistant', content: 'Now choose Civil or Criminal:' }])
    } else if (selectedCase && showNatureOptions) {
      // Call Gemini API for case analysis
      setLoading(true)
      try {
      ...
      try {
        ...
        setMessages((prev) => [...prev, { 
          role: 'assistant', 
          content: response.draftedStatement || 'I understand your concern. Let me help you with your legal case.' 
        }])
        ...
        setMessages((prev) => [...prev, { 
          role: 'assistant', 
          content: 'I understand your concern. Let me help you with your legal case.' 
        }])
      } finally {
        setLoading(false)
      }
  ...
  const handleNatureSelect = (nature: string) => {
    ...
    setMessages((prev) => [...prev, 
      ...
      { role: 'assistant', content: `Tell me about your case. You can send a voice recording or type it in here.` }
    ])
  }
  ...
  const handleDocumentUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast.success(`Successfully uploaded ${files.length} document(s)!`, { id: 'upload' })
    } catch (error) {
  ...
  return (
    <main className="grid place-items-center min-h-screen">
      <div className="w-[min(920px,92%)]">
        ...
        {messages.length > 0 && (
          <div className="main-container p-4 mb-4 h-[60vh] overflow-y-auto space-y-3" ref={(el) => {
            if (el) {
              el.scrollTop = el.scrollHeight;
            }
          }}>
            {messages.map((m, i) => (
              <div key={i} className={m.role === 'assistant' ? "p-3" : "flex justify-end"}>
                {m.role === 'user' ? (
                  <div className="inline-block text-gray-900 px-3 py-2">{m.content}</div>
                ) : (
            ...
            {showDocumentRequirements && (
              <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
                <h4 className="font-semibold text-blue-800 mb-2">Required Documents for {selectedCase} Case:</h4>
                <div className="text-sm text-blue-700 mb-3">
                  {selectedCase.toLowerCase().includes('divorce') ? (
                    <ul className="list-disc pl-5 space-y-1">
                      <li>Marriage certificate</li>
                      <li>Financial statements (bank statements, tax returns)</li>
            ...
            {showDocumentUpload && (
              <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
                <h4 className="font-semibold text-green-800 mb-2">Upload Your Documents:</h4>
                <p className="text-sm text-green-700 mb-3">
                  Upload whatever documents you have. You can select multiple files at once.
                </p>
            ...
            {showStatementRequest && (
              <div className="p-3 bg-purple-50 border border-purple-200 rounded-lg">
                <h4 className="font-semibold text-purple-800 mb-2">Tell Your Story:</h4>
                <p className="text-sm text-purple-700 mb-3">
                  Now can you submit a statement of what actually happened? You can either type it here or attach a voice recording.
                </p>
                <div className="flex gap-2">
                  <Button 
                    onClick={() => {
        ...
        />
          <div className="flex gap-2.5 mt-12">
            ...
            <div className="ml-auto flex items-center gap-2.5">
              ...
              <Button onClick={handleSend} className=" bg-blue-600 text-white" aria-label="Send"><ArrowUp size={18} /></Button>
            </div>
          </div>
        </div>
      </div>
